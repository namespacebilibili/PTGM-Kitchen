...
class GoalBasedAgent(BaseAgent):
    """Agent that acts based on Steve"""
    def __init__(self, config):
        super().__init__(config)
        self._update_model_params()     # transfer some parameters to model
    ...
    def _act(self, obs):
        split_obs = self._split_obs(obs)
        with no_batchnorm_update(self._policy) if obs.shape[0] == 1 else contextlib.suppress():
            actions = self._policy.act(split_obs)
        return actions
    
    def _split_obs(self, obs):
        assert obs.shape[1] == self._policy._hp.state_dim + self._policy._hp.goal_dim
        return AttrDict(
            states=torch.from_numpy(obs[:, :self._policy._hp.state_dim]).float().to(self._hp.device),   
            goals=torch.from_numpy(obs[:, -self._policy._hp.goal_dim:]).float().to(self._hp.device),
        )
